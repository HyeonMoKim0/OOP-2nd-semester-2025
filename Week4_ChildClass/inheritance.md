...
- 9번 문제처럼 다형성이 제대로 구현되지 않은 이유는 객체 잘림 현상이 발생했기 때문이다.
  ㆍ 자식 객체를 부모 객체에 값으로 복사할 때,     자식 클래스가 가진 멤버가 모두 잘려나가고 부모에 해당하는 멤버만 남는 현상을 말한다.
- 객체 잘림이 일어나는 이유는 메모리 크기와 관련이 있다.
  ㆍ 위의 코드에서 29줄에선 a2, 즉 Dog가 가진 크기를 Animal에 넣으려고 하는데, 이 둘의 크기는 다르다.
  ㆍ 만약에 Animal에는 int a라는 멤버만 존재하고, Dog에는 Animal의 멤버와 int b라는 멤버가 있다면,
     각각 4, 8Byte의 크기를 가지고 있다. 그런데 10줄처럼 컴파일러는 class Dog를 읽고 public Animal이라는     클래스를 읽게 되어 4줄로 넘어가서 해당 클래스에 있는 크기를 인지하고 메모리 공간을 확보하게 된다.
     그런데 실질적으로 29줄에 있는 b는 4Byte까지 할당하도록 자료형이 정의되어 있기 때문에, 컴파일러가
     읽는 순서에 따라 Animal에 있는 변수만을 위한 메모리 공간을 확보하게 된다. 그래서 객체 잘림 현상이
     발생하게 된다.
- 다형성은 이 객체의 실제 타입을 알아내서 그 실제 타입에 맞는 함수를 호출하게 된다.
  ㆍ 가상 함수 테이블(이하 vtable)과 가상 함수 포인터(이하 vptr)를 통해 구현된다.
  ㆍ 가상 함수가 하나라도 있는 클래스 객체에는 컴파일러가 vptr이라는 포인터를 추가하게 된다.
     vtable은 실제 어떤 가상 함수를 호출할 지에 대한 정보가 담긴 함수 주소 목록이기에, vptr은 해당     클래스의 vtable을 가리키게 된다.
  ㆍ 그래서 22줄 a2 객체의 vptr은 Dog의 vtable을 가리키고,     29줄 b 객체의 vptr은 Animal의 vtable을 가리킨다.
  ㆍ 따라서 컴파일러는 30줄에서 b가 담긴 a2의 주소를 찾아가 그곳에 있는 vptr,     즉 Dog를 가리키고 있기 때문에 Dog의 Speak() 함수를 출력하게 된다.
